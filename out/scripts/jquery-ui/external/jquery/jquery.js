...
if(jQuery.support.ownLast){ //noinspection LoopStatementThatDoesntLoopJS
for(key in obj){return core_hasOwn.call(obj,key);}} // Own properties are enumerated firstly, so to speed up,
// if last one is own, then all properties are own.
for(key in obj){}return key===undefined||core_hasOwn.call(obj,key);},isEmptyObject:function isEmptyObject(obj){var name; //noinspection LoopStatementThatDoesntLoopJS
for(name in obj){return false;}return true;},error:function error(msg){throw new Error(msg);}, // data: string of html
...
contains=rnative.test(docElem.contains)||docElem.compareDocumentPosition?function(a,b){var adown=a.nodeType===9?a.documentElement:a,bup=b&&b.parentNode; //noinspection JSBitwiseOperatorUsage
return a===bup||!!(bup&&bup.nodeType===1&&(adown.contains?adown.contains(bup):a.compareDocumentPosition&&a.compareDocumentPosition(bup)&16));}:function(a,b){if(b){while(b=b.parentNode){if(b===a){return true;}}}return false;}; /* Sorting
...
return sortInput?indexOf.call(sortInput,a)-indexOf.call(sortInput,b):0;} //noinspection JSBitwiseOperatorUsage
return compare&4?-1:1;} // Not directly comparable, sort on existence of method
...
if(!event.which&&button!==undefined){ //noinspection JSBitwiseOperatorUsage
event.which=button&1?1:button&2?3:button&4?2:0;}return event;}},special:{load:{ // Prevent triggered image.load events from bubbling to window.load
